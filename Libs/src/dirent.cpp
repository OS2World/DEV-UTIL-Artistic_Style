...
DIR *opendir(char *filename)
{
    ...
    len = strlen(filename);
    dirp->dirname =(char*) malloc(len + 5);
...
/*        if(strlen((const char*) ff.cchName) > 0)
           {

              if(!strncmp((const char*) ff.cchName, ".",0))
              continue;
           }

        if(strlen((const char*) ff.cchName) > 1)
           {
              if(strncmp((const char*) ff.cchName,"..",1))
              continue;
           }
*/
        ...
        if (dirp->tot_ent >= dirp->max_ent)
        {
            ...
            p =(dirent**) realloc(dirp->entp, dirp->max_ent * sizeof(entp));
            if (NULL == p)
            {
        ...
        entp = (dirent*) malloc(sizeof(*entp) + (size_t) ff.cchName);
        if (NULL == entp)
        {
            rc = ERROR_NOT_ENOUGH_MEMORY;
            break;
